...
async function verifyAdminAccess(supabaseAdmin, userId) {
  if (!userId) {
    return false;
  }

  const { data, error } = await supabaseAdmin.rpc('has_admin_access', { check_user_id: userId });
...
async function checkFlutterwaveCharge(payment) {
  try {
    ...
    console.error('admin-poll flutterwave check:', error?.message);
  }
  return null;
}
...
async function autoCancelPayment(admin, payment, reason) {
  if (payment.provider_transaction_id?.startsWith('chg_')) {
    try {
      ...
      console.error('admin-poll auto-cancel void:', voidError?.message);
    }
  }
  ...
  await admin.from('payment_attempt_diagnostics').insert({
    ...
    event_type: 'admin_auto_cancel_timeout',
    stage: 'cancelled',
...
async function markSuccessful(admin, payment, charge) {
  ...
  await admin.from('payment_attempt_diagnostics').insert({
    payment_id: payment.id,
    event_type: 'admin_poll_verified',
    stage: 'completed',
    provider_status: 'succeeded',
    internal_message: 'Payment confirmed by admin background poll.',
  });

  const { data: order } = await admin
    .from('orders')
    .select('id, reference, customer_id, quoted_amount_kobo, status')
...
export default {
  fetch: withSupabase({ auth: 'user' }, async (req, ctx) => {
    ...
    try {
      ...
      const isAdmin = await verifyAdminAccess(ctx.supabaseAdmin, userId);

      if (!isAdmin) {
        return Response.json({ success: false, message: 'Admin access required.' }, { status: 403 });
      }
      const { data: pendingPayments, error: fetchError } = await ctx.supabaseAdmin
        .from('payment_transactions')
        .select(`
          ...
          orders ( id, reference, customer_id, quoted_amount_kobo )
        `)
      ...
      for (const payment of pendingPayments) {
        ...
        if (!charge) {
          ...
          updated.push({ id: payment.id, status: 'pending' });
          continue;
        }
        ...
        if (localStatus === 'failed' || localStatus === 'cancelled') {
          ...
          updated.push({ id: payment.id, status: localStatus });
          continue;
        }
      ...
      console.error('admin-poll-payment-status:', error);

      return Response.json(
        { success: false, message: error instanceof Error ? error.message : 'Admin payment polling failed.' },
        { status: 500 },
      );
    }
  }),
